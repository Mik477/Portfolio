...
<script lang="ts">
  ...
  // NEW: Import the reusable bloom effect module.
  ...
  // --- DigitalDecayEffect Class with Bloom Integration ---
  ...
  class DigitalDecayEffect {
    ...
    // NEW: Property to hold the bloom effect instance.
    ...
    private SYMBOLS = [ '日', '〇', 'ハ', 'ミ', 'ヒ', 'ウ', 'シ', 'ナ', 'モ', 'サ', 'ワ', 'ツ', 'オ', 'リ', 'ア', 'ホ', 'テ', 'マ', 'ケ', 'メ', 'エ', 'カ', 'キ', 'ム', 'ユ', 'ラ', 'セ', 'ネ', 'ヲ', 'イ', 'ク', 'コ', 'ソ', 'タ', 'チ', 'ト', 'ノ', 'フ', 'ヘ', 'ヤ', 'ヨ', 'ル', 'レ', 'ロ', '∆', 'δ', 'ε', 'ζ', 'η', 'θ', '∃', '∄', '∅', 'Д' ];
    private SYMBOL_COLORS = [ new THREE.Color(0.0, 0.95, 0.05), new THREE.Color(0.0, 1.0, 0.0), new THREE.Color(0.3, 1.0, 0.3) ];
    ...
    private onImageLoaded(): void {
        this.setupScene();
        // NEW: Instantiate the BloomEffect after the scene is ready.
    ...
    private animate(): void {
        ...
        // MODIFIED: The render call is now handled by the bloom effect's composer.
    ...
    private onWindowResize(): void {
        ...
        // NEW: Ensure the bloom effect's composer is also resized.
        if (this.bloomEffect) {
    ...
    public dispose(): void {
      ...
      // NEW: Dispose of the bloom effect resources.
    ...
    // --- All other methods from the previous version remain unchanged and are omitted for brevity ---
    // (start, stop, beginLeaveAnimation, createParticleSystem, fullReset, etc.)
    public start(): void { if (this.animationFrameId === null) { this.fullReset(); this.clock = new THREE.Clock(); this.animate(); } }
    ...
    private fullReset(): void { this.isFadingOut = false; this.fadeOutTimer = 0; if(this.particleSystem) { this.particleSystem.material.uniforms.globalOpacity.value = 1.0; } this.resetBlackoutGrid(); this.resetAllParticles(); }
    ...
    private updateGrid(deltaTime: number): void { if (this.particles.active.size >= this.MAX_ACTIVE_PARTICLES) return; const spawnCols = Math.floor(this.grid.cols * this.SPAWN_AREA_WIDTH); for (let i = 0; i < this.grid.cells.length; i++) { const cell = this.grid.cells[i]; const fadeSpeed = deltaTime / this.BLACKOUT_FADE_DURATION; if (cell.state === this.STATE.ACTIVE) { cell.opacity = Math.min(1.0, cell.opacity + fadeSpeed); } else { cell.opacity = Math.max(0.0, cell.opacity - fadeSpeed); } if (cell.state === this.STATE.IDLE) { const col = i % this.grid.cols; if (col < spawnCols) { const normalizedX = col / spawnCols; const probability = this.BASE_CHANCE * Math.exp(-this.PROBABILITY_DECAY_FACTOR * normalizedX) * deltaTime * 60; if (Math.random() < probability) { cell.state = this.STATE.ACTIVE; this.spawnParticleForCell(i); } } } else if (cell.state === this.STATE.REGENERATING) { cell.timer += deltaTime; if (cell.timer >= this.REGENERATION_TIME && cell.opacity <= 0) { cell.state = this.STATE.IDLE; cell.timer = 0; } } } }
